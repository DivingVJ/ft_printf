...
int	check_format(const char *str, va_list argptr)
{
	...
	if (*str == 'p')
	{
		...
		len = len + ft_putnbr_base_fd((unsigned long)va_arg(argptr, void *), base, 1);
	}
	if ((*str == 'd') || (*str == 'i'))
	...
	if (*str == 'x')
		len = ft_putnbr_base_fd(va_arg(argptr, unsigned int), base, 1);
	if (*str == 'X')
		len = ft_putnbr_base_fd(va_arg(argptr, unsigned int), cbase, 1);
	if (*str == '%')
		len = ft_putchar_fd('%', 1);
	return (len);
}
